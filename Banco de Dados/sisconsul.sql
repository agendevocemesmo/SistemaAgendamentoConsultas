...
CREATE SCHEMA IF NOT EXISTS `sisconsul` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `sisconsul` ;

-- -----------------------------------------------------
-- Table `logSystem`.`usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sisconsul`.`usuario` (
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sisconsul`.`senha` (
  `us_cod` INT NOT NULL,
  ...
  CONSTRAINT `fk_senha_usuario`
    FOREIGN KEY (`us_cod`)
    REFERENCES `sisconsul`.`usuario` (`us_cod`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
